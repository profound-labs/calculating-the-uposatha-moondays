...
\item Adhikamāsa years are reliable to predict. Some adhikavāra years have been
...
or ask the machine to do it in Golang at sec. \ref{suriya-go-example}.
\par}
...
adhikavāra is carried on the following year.}
...
\marginpar{%
Only basic operations with pen and paper are necessary to get these results.
(Although we'll get the machine to do it for us in the end.)

Doing astronomy without looking up the sky does mean there may be inaccuracies
for a given day, but the long-term calendar remains in sync.
...
information to describe the progression of these events and plan any calendar
year.
}
...
\begin{description}
\item[{MeanSun}] Mean position of the Sun
\item[{TrueSun}] True position of the Sun
\item[{MeanMoon}] Mean position of the Moon
\item[{TrueMoon}] True position of the Moon
\item[{Raek\footnote{a.k.a. Naksatra}}] The position of the Moon in the 27 mansions
of the Zodiac, which will determine the month
\end{description}
...
in the correct month, and not at some other Full Moon.
...
$\lfloor a \rfloor$ \emph{floors} (or truncates) a fraction value, meaning we only
...
\subsubsection{Adhikamāsa years}
\label{sec-3-1-2-1}
...
\subsubsection{Adhikavāra years}
\label{sec-3-1-2-2}
...
\subsubsection{Sun, Moon and Raek}
\label{sec-3-1-2-3}
...
\begin{table}[p]
\caption{\label{tbl-calendars-1958} Adhikamāsa and adhikavāra in the period 1958 to 1978 (CS 1320-1340).\cite{eade-interpolation}}\legend{m for adhikamāsa, d for adhikavāra years, \Delta m and \Delta d for years since last adhikamāsa and adhikavāra.}
\centering
