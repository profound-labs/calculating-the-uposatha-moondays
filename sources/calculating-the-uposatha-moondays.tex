...
\begin{split}
                      a &= \mathbf{elapsedDays} * \mathbf{800} + \mathbf{Year\_Kammacubala}\\
                        &= x\\
                      b &= (a / \mathbf{292207}) * 360\\
                        &= x\\
                  x, y, z &= \mathit{DegreeToRal}(b)\\
                        z &= z - 3\\
         \mathbf{MeanSun} &= \mathit{RalToDegree}(x, y, z)
\end{split}\\
\begin{split}
                         a &= | \mathbf{MeanSun} - 80 | \\
                           &= x\\
                         b &= \lfloor 134 * \mathit{sin}(a_{rad}) \rfloor\\
          \mathbf{TrueSun} &= \mathbf{MeanSun} + b/60
\end{split}
\end{align}
...
\begin{split}
  ...
  \mathbf{MeanMoon} &= \mathit{NormalizeDegree}( \mathbf{TrueSun} + a + \mathbf{Tithi} * 12 - \mathit{RalToDegree}(0, 0, 40) )\\
                    &= x
\end{split}
\end{align}

One *Tithi* is 12\degree, $360\degree / 30 = 12\degree$.
